                                ...
                                <c:when test="${currentHd != null}">
                                    <form action="${pageContext.request.contextPath}/ban-hang" method="POST" class="needs-validation" novalidate onsubmit="return validateThanhToanPOS(event, ${empty chiTiets ? 0 : chiTiets.size()})">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <!-- Giá trị voucher thực tế được lưu ở đây -->
                                                <input type="hidden" id="selectPgg" value="${currentHd.phieuGiamGia != null ? currentHd.phieuGiamGia.id : ''}">
                                                
                                                <button type="button" class="btn btn-outline-danger w-100 fw-bold d-flex align-items-center justify-content-center gap-2" style="border-style: dashed;" onclick="new bootstrap.Modal(document.getElementById('modalVoucherPicker')).show()">
                                                    <i class="bi bi-ticket-perforated"></i> 
                                                    <c:choose>
                                                        <c:when test="${currentHd.phieuGiamGia != null}">
                                                            Đã áp dụng: ${currentHd.phieuGiamGia.maPhieu}
                                                        </c:when>
                                                        <c:otherwise>
                                                            Chọn Mã Giảm Giá
                                                        </c:otherwise>
                                                ...
                                                <c:if test="${currentHd.phieuGiamGia != null}">
                                                    <button type="button" class="btn btn-danger" title="Gỡ Voucher" onclick="document.getElementById('selectPgg').value=''; apDungVoucherPOS(${currentHd.id});">
                                ...
                                <c:forEach var="pgg" items="${phieuGiamGias}">
                                    ...
                                    <div class="voucher-ticket ${!duDieuKien ? 'disabled' : ''} ${isDangApDung ? 'active' : ''}" 
                                        ...
                                        <div class="voucher-right position-relative">
                                            ...
                                            <div class="small text-muted mb-2">Đơn tối thiểu <fmt:formatNumber value="${pgg.dieuKienGiam}" type="number"/>đ</div>
